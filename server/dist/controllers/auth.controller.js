...
exports.ensureEmailAccount = ensureEmailAccount;
exports.login = login;
exports.me = me;
...
async function register(req, res) {
    try {
        ...
        try {
            let freePlan = await prisma_1.default.plan.findFirst({
                where: {
                    priceMonthly: 0,
                    isActive: true
                },
            ...
            if (!freePlan) {
                ...
                freePlan = await prisma_1.default.plan.create({
                    data: {
                        name: "Gratis",
        ...
        try {
            ...
            let domain = await prisma_1.default.domain.findFirst({
                where: { domainName },
            });
            if (!domain) {
                ...
                try {
                    domain = await prisma_1.default.domain.create({
                        data: {
                            domainName,
                            userId: user.id,
                            dnsVerified: true,
                    ...
                    if (createError.code === "P2002" || createError.message?.includes("Unique constraint")) {
                        ...
                        domain = await prisma_1.default.domain.findFirst({
                            where: { domainName },
                        });
                        if (domain) {
            ...
            const existingAccount = await prisma_1.default.emailAccount.findUnique({
            ...
            if (existingAccount) {
                ...
                const { default: SmtpUserService } = await Promise.resolve().then(() => __importStar(require("../services/smtp-user.service")));
                const smtpResult = await SmtpUserService.createSmtpUser(finalEmail, password, // Contraseña sin hashear para el servidor SMTP
                domain.domainName);
                ...
                const verifyAccount = await prisma_1.default.emailAccount.findUnique({
                    where: { id: emailAccount.id },
            ...
            const allUserAccounts = await prisma_1.default.emailAccount.findMany({
            ...
            if (allUserAccounts.length > 0) {
                ...
                try {
                    const welcomeEmail = await prisma_1.default.email.create({
                        data: {
  ...
  <div class="container">
    ...
    <div class="footer">
      ...
      <p style="margin-top: 10px;">El equipo de Fylo Mail</p>
        ...
        const token = jsonwebtoken_1.default.sign({ id: user.id }, JWT_SECRET, { expiresIn: JWT_EXPIRES });
...
async function ensureEmailAccount(req, res) {
    try {
        const user = req.user;
        ...
        let account = await prisma_1.default.emailAccount.findFirst({
            where: { ownerId: user.id },
            select: { id: true, address: true, ownerId: true },
        });
        if (account) {
        ...
        account = await prisma_1.default.emailAccount.findUnique({
        ...
        const userWithPassword = await prisma_1.default.user.findUnique({
            where: { id: user.id },
        ...
        const newAccount = await prisma_1.default.emailAccount.create({
            data: {
                address: user.email,
        ...
        const smtpResult = await SmtpUserService.createSmtpUser(user.email, tempPassword, // En producción, esto debería venir del request o generarse
        domain.domainName);
        if (smtpResult.success) {
            // Actualizar la cuenta con la configuración SMTP
            await prisma_1.default.emailAccount.update({
...
async function me(req, res) {
    try {
        ...
        const rawUser = await prisma_1.default.$queryRaw `
...
async function changePassword(req, res) {
    try {
        ...
        const isPasswordValid = await (0, hash_1.comparePassword)(currentPassword, user.password);
        if (!isPasswordValid) {
...
async function forgotPassword(req, res) {
    try {
        ...
        const resetToken = jsonwebtoken_1.default.sign({ userId: user.id, type: "password-reset" }, JWT_SECRET, { expiresIn: "1h" });
