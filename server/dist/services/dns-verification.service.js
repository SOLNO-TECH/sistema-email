...
class DnsVerificationService {
     ...
     */
    static async verifyMX(domainName) {
        try {
            const mxRecords = await promises_1.default.resolveMx(domainName);
     ...
     */
    static async verifySPF(domainName) {
        try {
            const txtRecords = await promises_1.default.resolveTxt(domainName);
     ...
     */
    static async verifyDKIM(domainName, selector = "default") {
        try {
            const dkimDomain = `${selector}._domainkey.${domainName}`;
            const txtRecords = await promises_1.default.resolveTxt(dkimDomain);
     ...
     */
    static async verifyDMARC(domainName) {
        try {
            const dmarcDomain = `_dmarc.${domainName}`;
            const txtRecords = await promises_1.default.resolveTxt(dmarcDomain);
     ...
     */
    static getDnsInstructions(domainName) {
        return {
            mx: [
                `Registro MX:`,
                `  Nombre: ${domainName}`,
...
exports.default = DnsVerificationService;
