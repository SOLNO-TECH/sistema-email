...
class EmailSenderService {
     ...
     */
    static async getSmtpConfig(emailAccountId) {
        ...
        throw new Error("SMTP no configurado. Configura EMAIL_SMTP_HOST, EMAIL_SMTP_USER y EMAIL_SMTP_PASSWORD en .env para envío automático desde cualquier email.");
    }
    /**
        ...
        switch (config.provider) {
            ...
            default:
                ...
                if (!config.user || !config.password) {
                    throw new Error("SMTP no configurado. Configura EMAIL_SMTP_USER y EMAIL_SMTP_PASSWORD en .env o configura SMTP para el dominio/cuenta");
                }
                return nodemailer_1.default.createTransport({
     ...
     */
    static async sendFromAccount(emailAccountId, to, subject, message) {
        try {
            const account = await prisma_1.default.emailAccount.findUnique({
                where: { id: emailAccountId },
            });
     ...
     */
    static async verifyAccountSmtp(emailAccountId) {
        try {
            const smtpConfig = await this.getSmtpConfig(emailAccountId);
            const transporter = this.createTransporter(smtpConfig);
