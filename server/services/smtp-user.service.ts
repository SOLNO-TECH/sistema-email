...
interface CreateSmtpUserResult {
  success: boolean;
  error?: string;
  smtpHost?: string;
  smtpPort?: number;
...
class SmtpUserService {
   ...
   */
  static async createSmtpUser(
    email: string,
    password: string,
    domain: string
  ): Promise<CreateSmtpUserResult> {
    try {
   ...
   */
  private static async createLocalSmtpUser(
    email: string,
    password: string,
    domain: string
  ): Promise<CreateSmtpUserResult> {
   ...
   */
  private static async createRemoteSmtpUser(
    email: string,
    password: string,
    domain: string,
    smtpHost: string
    ...
    try {
      ...
      if (smtpApiUrl) {
        ...
        return new Promise((resolve, reject) => {
          const req = (apiUrl.protocol === "https:" ? https : http).request(options, (res: any) => {
            let data = "";
   ...
   */
  static async getSmtpConfigForAccount(emailAccountId: number): Promise<{
    smtpHost: string;
    smtpPort: number;
    smtpUser: string;
    smtpPassword: string;
   ...
   */
  static async deleteSmtpUser(email: string, domain: string): Promise<boolean> {
    try {
      const smtpHost = process.env.EMAIL_SMTP_HOST;
  ...
  private static async deleteLocalSmtpUser(email: string, domain: string): Promise<boolean> {
    try {
      const postfixVirtualFile = process.env.POSTFIX_VIRTUAL_FILE || "/etc/postfix/virtual";
      const postfixVirtualMailboxFile = process.env.POSTFIX_VIRTUAL_MAILBOX_FILE || "/etc/postfix/virtual_mailbox";
  ...
  private static async deleteRemoteSmtpUser(email: string, domain: string, smtpHost: string): Promise<boolean> {
    try {
      const smtpApiUrl = process.env.EMAIL_SMTP_API_URL;
...
export default SmtpUserService;
