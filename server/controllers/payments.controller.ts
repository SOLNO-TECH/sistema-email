...
export async function createPaymentIntent(req: any, res: Response) {
  try {
    const user = req.user;
...
export async function processCardPayment(req: any, res: Response) {
  try {
    const user = req.user;
...
export async function processPayPalPayment(req: any, res: Response) {
  try {
    const user = req.user;
...
export async function createBankTransferSubscription(req: any, res: Response) {
  try {
    const user = req.user;
...
export async function addPaymentMethod(req: any, res: Response) {
  try {
    const user = req.user;
...
export async function listPaymentMethods(req: any, res: Response) {
  try {
    const user = req.user;
    ...
    if (stripe && details.stripeCustomerId) {
      try {
        ...
        const combinedMethods = stripeMethods.data.map((pm: any) => ({
          id: pm.id,
          type: "card",
          last4: pm.card?.last4,
          brand: pm.card?.brand,
        ...
        const paypalMethods = paymentMethods.filter((pm: any) => pm.type === "paypal");
...
export async function deletePaymentMethod(req: any, res: Response) {
  try {
    const user = req.user;
    ...
    paymentMethods = paymentMethods.filter((pm: any) => {
...
export async function setDefaultPaymentMethod(req: any, res: Response) {
  try {
    const user = req.user;
    ...
    const methodExists = paymentMethods.some((pm: any) => {
      return pm.stripePaymentMethodId === paymentMethodId ||
...
export async function addCredits(req: any, res: Response) {
  try {
    const user = req.user;
    ...
    if (defaultMethodId) {
      selectedMethod = paymentMethods.find((pm: any) => {
