...
export async function updateRecoverySettings(req: Request, res: Response) {
  try {
    const userId = (req as any).user.id;
    const {
      recoveryEmail,
    ...
    if (recoveryPhone !== undefined) updateData.recoveryPhone = recoveryPhone;
    if (recoveryPhoneCountryCode !== undefined) updateData.recoveryPhoneCountryCode = recoveryPhoneCountryCode;
    if (allowEmailRecovery !== undefined) updateData.allowEmailRecovery = allowEmailRecovery;
    if (allowPhoneRecovery !== undefined) updateData.allowPhoneRecovery = allowPhoneRecovery;
    if (allowQRLogin !== undefined) updateData.allowQRLogin = allowQRLogin;
    if (allowRecoveryPhrase !== undefined) updateData.allowRecoveryPhrase = allowRecoveryPhrase;
    if (allowDeviceRecovery !== undefined) updateData.allowDeviceRecovery = allowDeviceRecovery;
...
export async function updateSecuritySettings(req: Request, res: Response) {
  try {
    ...
    const updateData: any = {};
    if (twoPasswordMode !== undefined) updateData.twoPasswordMode = twoPasswordMode;
...
export async function applyGiftCode(req: Request, res: Response) {
  try {
    const userId = (req as any).user.id;
    const { code } = req.body;
...
export async function removeEmergencyContact(req: Request, res: Response) {
  try {
    const userId = (req as any).user.id;
    const { email } = req.body;
...
export async function deleteAccount(req: Request, res: Response) {
  try {
    ...
    const isDefaultAdmin = user.role === "admin" && (
      (firstAdmin && firstAdmin.id === user.id) ||
...
export async function generateRecoveryPhrase(req: Request, res: Response) {
  try {
    const userId = (req as any).user.id;
...
export async function setupDeviceRecovery(req: Request, res: Response) {
  try {
    const userId = (req as any).user.id;
